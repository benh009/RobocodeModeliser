...
import com.mxgraph.examples.config.SCXMLConstraints.RestrictedState.PossibleAction;
...
public class SCXMLNode implements Serializable {
    ...
    public List<PossibleInformation> getPossibleInformations() {
        List<PossibleInformation> possibleInformation = null;
        if (isRestricted()) {
            possibleInformation = new LinkedList<PossibleInformation>();
            for (RestrictedState tempState : getRestrictedStates()) {
                possibleInformation.addAll(tempState.getPossibleInformation());
            }
        }
        return possibleInformation;
    }
        public List<PossibleAction> getPossibleActions() {
        List<PossibleAction> possibleAction = null;
        if (isRestricted()) {
            possibleAction = new LinkedList<PossibleAction>();
            for (RestrictedState tempState : getRestrictedStates()) {
                possibleAction.addAll(tempState.getPossibleAction());
            }
        }
